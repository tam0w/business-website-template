...
const InvertedRippleGrid: React.FC<Props> = ({
  enableRainbow = false,
  gridColor = '#ffffff',
...
void main() {
    ...
    // INVERTED: Create edge mask - stronger at edges, weaker at center
    vec2 edgeCoords = vUv - 0.5;
    float distFromCenter = length(edgeCoords);

    // Create a mask that is 0 at center and 1 at edges
    float edgeMask = pow(distFromCenter * 2.0, vignetteStrength);
    edgeMask = clamp(edgeMask, 0.0, 1.0);

    // Apply radial fade from center outward (inverted from original)
    float radialFade = 1.0 - exp(-2.0 * clamp(pow(dist, fadeDistance), 0.0, 1.0));
    ...
    // Combine edge mask with radial fade to show patterns only at edges
    float finalFade = radialFade * edgeMask;
  ...
  return <div ref={containerRef} className="w-full h-full absolute inset-0 overflow-hidden [&_canvas]:block" />;
};

export default InvertedRippleGrid;
